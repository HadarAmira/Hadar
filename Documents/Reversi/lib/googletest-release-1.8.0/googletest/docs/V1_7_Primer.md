...
_Note:Z We sometimes refer to Google C++ Testing Framework informally
as _Google Test_.
...
test case. Both names must be valid C++ identifiers, and they should not contain underscore (`Z`). A test's _full name_ consists of its containing test case and its
