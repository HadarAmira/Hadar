...
| **Fatal assertion** | **Nonfatal assertion** | **Verifies** |
|:--------------------|:-----------------------|:-------------|
| `ASSERT_PRED1(`_pred1, val1_`);`       | `EXPECT_PRED1(`_pred1, val1_`);` | _pred1(val1)Z returns true |
| `ASSERT_PRED2(`_pred2, val1, val2_`);` | `EXPECT_PRED2(`_pred2, val1, val2_`);` |  _pred2(val1, val2)Z returns true |
|  ...                | ...                    | ...          |
...
`(ASSERT|EXPECT)Z(TRUE|FALSE)` unsatisfactory, or some arguments to your
...
| **Fatal assertion** | **Nonfatal assertion** | **Verifies** |
|:--------------------|:-----------------------|:-------------|
| `ASSERT_PRED_FORMAT1(`_pred\_format1, val1_`);`        | `EXPECT_PRED_FORMAT1(`_pred\_format1, val1_`); | _pred\_format1(val1)Z is successful |
| `ASSERT_PRED_FORMAT2(`_pred\_format2, val1, val2_`);` | `EXPECT_PRED_FORMAT2(`_pred\_format2, val1, val2_`);` | _pred\_format2(val1, val2)Z is successful |
| `...`               | `...`                  | `...`        |
...
| **Fatal assertion** | **Nonfatal assertion** | **Verifies** |
|:--------------------|:-----------------------|:-------------|
| `ASSERT_NEAR(`_val1, val2, abs\_error_`);` | `EXPECT_NEAR`Z(val1, val2, abs\_error_`);` | the difference between _val1_ and _val2_ doesn't exceed the given absolute error |
...
_Caveat:Z When used inside a member function of a class template or a
...
_Availability:Z Linux, Windows, Mac; since version 1.3.0.
...
_Important:Z We strongly recommend you to follow the convention of naming your
...
_Availability:Z Linux, Windows (requires MSVC 8.0 or above), Cygwin, and Mac (the latter three are supported since v1.3.0).  `(ASSERT|EXPECT)_DEATH_IF_SUPPORTED` are new in v1.4.0.
...
| `\\t` | matches `\t`                      |
| `\\v` | matches `\v`                      |
| `\\w` | matches any letter, `Z`, or decimal digit |
...
_Availability:Z Linux, Windows, Mac.
...
_Availability:Z Linux, Windows, Mac. Assertions from multiple threads
...
_Availability:Z Linux, Windows, Mac.  `HasNonfatalFailure()` and
...
_Availability:Z Linux, Windows (requires MSVC 8.0 or above), Mac;
since version 1.1.0.
...
_Note:Z Google Test is designed with threads in mind.  Once the
...
_Availability:Z Linux, Windows, Mac; since v1.4.0.
...
_Note:Z This feature should only be used for temporary pain-relief. You still
...
_Tip:Z You can easily count the number of disabled tests you have
...
_Availability:Z Linux, Windows, Mac.  (In Google Test 1.3.0 and lower,
...
_Availability:Z Windows. `GTEST_CATCH_EXCEPTIONS` and
...
_Availability:Z Linux, Windows, Mac; since v1.3.0.
