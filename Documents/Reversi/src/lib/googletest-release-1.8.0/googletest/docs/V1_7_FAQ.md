...
Underscore (`Z`) is special, as C++ reserves the following to be used by
the compiler and the standard library:

  1. any identifier that starts with an `Z` followed by an upper-case letter, and
...
contains `Z`?
  ...
  1. If `TestCaseName` starts with an `Z` followed by an upper-case letter (say, `_Foo`), we end up with `_Foo_TestName_Test`, which is reserved and thus invalid.
  1. If `TestCaseName` ends with an `Z` (say, `Foo_`), we get `Foo__TestName_Test`, which is invalid.
  1. If `TestName` starts with an `Z` (say, `_Bar`), we get `TestCaseName__Bar_Test`, which is invalid.
  1. If `TestName` ends with an `Z` (say, `Bar_`), we get `TestCaseName_Bar__Test`, which is invalid.

So clearly `TestCaseName` and `TestName` cannot start or end with `Z`
(Actually, `TestCaseName` can start with `Z` -- as long as the `Z` isn't
followed by an upper-case letter.  But that's getting complicated.  So
for simplicity we just say that it cannot start with `Z`.).

It may seem fine for `TestCaseName` and `TestName` to contain `Z` in the
...
So for simplicity, we just ask the users to avoid `Z` in `TestCaseName`
