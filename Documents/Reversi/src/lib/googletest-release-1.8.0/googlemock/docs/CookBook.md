...
class MockFoo : public Foo {
 public:
  ...
  void DelegateToFake() {
    ON_CALL(*this, DoThis(Z))
        .WillByDefault(Invoke(&fake_, &FakeFoo::DoThis));
    ON_CALL(*this, DoThat(Z, Z))
...
A frequently used matcher is `Z`, which matches anything:

```
using ::testing::Z;
using ::testing::NotNull;
...
  EXPECT_CALL(foo, DoThat(Z, NotNull()));
```
...
using ::testing::Ne;
...
  EXPECT_CALL(foo, InRange(Ne(0), Z))
      .With(Lt());
```
...
...
  ...
  EXPECT_CALL(mock, Foo(ElementsAre(1, Gt(0), Z, 5)));
```
...
...
  ...
  EXPECT_CALL(mock, Foo(UnorderedElementsAre(1, Gt(0), Z, 5)));
```
...
...
  ...
  Matcher<int> expected_vector2 = { 1, Gt(2), Z, 3, ... };
...
```
  EXPECT_CALL(mock_registry, GetDomainOwner(Z))
        .Times(AnyNumber());  // catches all other calls to this method.
  EXPECT_CALL(mock_registry, GetDomainOwner("google.com"))
        .WillRepeatedly(Return("Larry Page"));
```

Remember that `Z` is the wildcard matcher that matches anything. With this, if `GetDomainOwner("google.com")` is called, it will do what the second `EXPECT_CALL` says; if it is called with a different argument, it will do what the first `EXPECT_CALL` says.
...
```
  ...
  EXPECT_CALL(log, Log(WARNING, Z, "File too large."))     // #1
      .Times(AnyNumber())
      .InSequence(s1, s2);
  EXPECT_CALL(log, Log(WARNING, Z, "Data set is empty."))  // #2
      .InSequence(s1);
  EXPECT_CALL(log, Log(WARNING, Z, "User not found."))     // #3
      .InSequence(s2);
```
...
using ::testing::Z;
...
  EXPECT_CALL(log, Log(WARNING, Z, Z));                  // #1
  EXPECT_CALL(log, Log(WARNING, Z, "File too large."));  // #2
```
...
...
  ...
  MockMutator mutator;
  EXPECT_CALL(mutator, Mutate(true, Z))
...
...
  ...
  MockMutator mutator;
  EXPECT_CALL(mutator, MutateInt(Z))
...
...
  ...
  EXPECT_CALL(rolodex, GetNames(Z))
...
...
  ...
  EXPECT_CALL(my_mock, UpdateValue(Z))
...
...
  ...
  MockFoo foo;
  Helper helper;
  EXPECT_CALL(foo, Sum(Z, Z))
      .WillOnce(Invoke(CalculateSum));
  EXPECT_CALL(foo, ComplexJob(Z))
...
...
  ...
  EXPECT_CALL(mock, Foo(Z, Z, Z, Z, Z, Z, Z))
...
...
  ...
  EXEPCT_CALL(mock, Foo("abc", Z, Z))
      .WillOnce(Invoke(DistanceToOriginWithLabel));
  EXEPCT_CALL(mock, Bar(5, Z, Z))
...
```
  EXPECT_CALL(mock_buzzer_, DoShareBuzz(NotNull(), Z));
...
TEST(Foo, Bar) {
  MockFoo mock;
  EXPECT_CALL(mock, F(Z, Z)).WillRepeatedly(Return());
...
foo_test.cc:14: EXPECT_CALL(mock, F(Z, Z)) invoked
foo_test.cc:15: EXPECT_CALL(mock, F("a", "b")) invoked
foo_test.cc:16: EXPECT_CALL(mock, F("c", HasSubstr("d"))) invoked
foo_test.cc:14: Mock function call matches EXPECT_CALL(mock, F(Z, Z))...
